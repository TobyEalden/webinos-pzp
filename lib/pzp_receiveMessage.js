...
var PzpReceiveMessage = function () {
    ...
    function fetchJS(msg){
        ...
        for (var i = 0, len = serviceCache.length; i < len; i++){
            var serviceLookup;
            if (serviceCache[i].apiURI && msg.payload && msg.payload.message && serviceCache[i].apiURI.length < msg.payload.message.length) {
                serviceLookup = msg.payload.message.substr(0, serviceCache[i].apiURI.length);
            } else {
                serviceLookup =  msg.payload.message;
            }
            if (serviceCache[i].apiURI === serviceLookup){
                var fileList = PzpCommon.fs.readdirSync(PzpCommon.path.join(serviceCache[i].path, "wrt"));
                fileList.sort();
                var proxyJS = "";
                for (var j = 0; j < fileList.length; j = j + 1) {
                    var proxyPath = PzpCommon.path.join(serviceCache[i].path, "wrt", fileList[j]);
                    var stat = PzpCommon.fs.statSync(proxyPath);
                    if (stat.isFile()) {
                        var data = PzpCommon.fs.readFileSync(proxyPath);
                        proxyJS = proxyJS + data.toString();
                    }
                }

                if (proxyJS.length > 0) {
                    var serviceName = serviceCache[i].apiURI.split("/");
    ...
    function clientJS(msg){
        PzpCommon.fs.writeFileSync(PzpCommon.path.join(PzpCommon.wUtil.webinosPath.webinosPath(), "wrt", msg.payload.message.name+".js"), msg.payload.message.js);
        PzpCommon.wUtil.webinosService.createWebinosJS(PzpCommon.path.join(__dirname, "../node_modules"), PzpObject.getOwnModules());
    }
