        ...
        if (syncInstance) {
           //var policyFile = PzpCommon.fs.readFileSync(PzpCommon.path.join(PzpObject.getMetaData("webinosRoot"), "policies", "policy.xml"));
            ...
            if (typeof PzpObject.notificationManager !== "undefined") {
              list["notifications"] = PzpObject.notificationManager.getNotifications();
              // Always get the latest config from the PZH.
              list["notifications"].config = {};
            }
        }
        return list;
    }
                    ...
                    for (var key in list){
                        if (list.hasOwnProperty(key)){
                            if (key === "trustedList"&& JSON.stringify(PzpObject.getTrustedList()) !== JSON.stringify(list[key])) {
                              ...
                              if (PzpObject.notificationManager.updateAfterSync(list[key],true)) {
