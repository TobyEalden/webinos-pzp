    ...
    this.fetchProxyJS = function() {
      if (PzpObject.getState("Pzh") === "connected") {
        ...
        remoteServiceCache.forEach(function(service){
          ...
          var found = ownServices.some(function(ownService){
            return ownService.api && ownService.api === service.api;
          });

          if (found !== true){
            // Service not found locally - check if proxy javascript file has already been downloaded.
            var fileNames = PzpCommon.fs.readdirSync(PzpCommon.path.join(PzpCommon.wUtil.webinosPath.webinosPath(), "wrt"));
            fileNames.sort();
            // ToDo - this is broken for some APIS - e.g. http://webinos.org/api/sensors/temperature
            var serviceName = service.api.split("/");
            serviceName = serviceName[serviceName.length-1]+".js";
            if (fileNames.indexOf(serviceName) === -1) {
              if (!notPresentServices.hasOwnProperty(service.api)) {
                notPresentServices[service.api] = {};
              }
              notPresentServices[service.api][service.serviceAddress] = true;
            }
          }
        });

        if (Object.keys(notPresentServices).length > 0){
          for (var missingApi in notPresentServices){
            if (notPresentServices.hasOwnProperty(missingApi)) {
              for (var serviceAddress in notPresentServices[missingApi]) {
                if (notPresentServices[missingApi].hasOwnProperty(serviceAddress)) {
                  PzpObject.prepMsg(serviceAddress, "fetchJS", missingApi);
                }
              }
            }
          }
        ...
        if (cert && conn.authorized /*&& (config.cert.validateConnection(cert.issuer.CN, getCAList()))*/) {
            ...
            if (type === "Pzh") {
              ...
              process.nextTick(function() {
                PzpObject.trackEntity(PzpObject.getPzhId());
                PzpObject.fetchProxyJS();
              });
            }
        } else {
