...
var PzpOtherManager = function () {
    ...
    var peerDiscovery;
    var PzpObject = this;
    var notificationSyncInterval = 10000;

    function initialize_PolicyManager(rpcHandler, notificationManager) {
        try {
            if (require.resolve("webinos-policy")) {
                ...
                pm.getPolicyManager().initialisePrompts(notificationManager);
            }
        } catch(err){
    ...
    this.startOtherManagers = function (){
        ...
        initialize_PolicyManager(rpcHandler, PzpObject.notificationManager);
